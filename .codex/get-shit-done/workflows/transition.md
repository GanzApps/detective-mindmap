...
```bash
TRANSITION=$(node "C:/workspace/ai-learn/3d graph mindmap/.codex/get-shit-done/bin/gsd-tools.cjs" phase complete "${current_phase}")
```
...
```bash
PROGRESS=$(node "C:/workspace/ai-learn/3d graph mindmap/.codex/get-shit-done/bin/gsd-tools.cjs" progress bar --raw)
```
...
if [ -n "$GSD_WORKSTREAM" ]; then
  WS_LIST=$(node "C:/workspace/ai-learn/3d graph mindmap/.codex/get-shit-done/bin/gsd-tools.cjs" workstream list --raw)
fi
```
...
```bash
node "C:/workspace/ai-learn/3d graph mindmap/.codex/get-shit-done/bin/gsd-tools.cjs" config-set workflow._auto_chain_active false
```
